...
<!--        >导出</el-button>-->
<!--      </el-col>-->
	  <right-toolbar :showSearch.sync="showSearch" @queryTable="getReLists"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" ref="financialLists" border :data="financialLists" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="事情审批id" style="text-align: center ;" prop="approvalId"   v-if="show"/>
      <el-table-column label="项目编号" style="text-align: center ;" prop="proNo"  v-if="show" />
      <el-table-column label="项目名称" style="text-align: center ;"  prop="ctBuInvoiceReqVo.proName_" :show-overflow-tooltip="true"/>
      <el-table-column label="合同名称" style="text-align: center ;"  prop="ctBuInvoiceReqVo.contractName_" :show-overflow-tooltip="true"/>

      <el-table-column label="申请部门" style="text-align: center ;"  prop="ctBuInvoiceReqVo.reqDeptname_" :show-overflow-tooltip="true"/>
      <el-table-column label="申请人" style="text-align: center ;"  prop="ctBuInvoiceReqVo.reqUsername_" />
      <el-table-column label="申请日期" style="text-align: center ;"  prop="ctBuInvoiceReqVo.reqDate_" :show-overflow-tooltip="true"/>
      <el-table-column label="付款单位名称" width="150px" style="text-align: center ;"  prop="ctBuInvoiceReqVo.payerName_" :show-overflow-tooltip="true"/>
      <el-table-column label="开票金额" style="text-align: center ;"  prop="ctBuInvoiceReqVo.invoiceAmountTax_" />

      <el-table-column label="发票代码" style="text-align: center ;" prop="invoiceCode" />
      <el-table-column label="发票号码" style="text-align: center ;" prop="invoiceNo" />
      <el-table-column label="回款时间" style="text-align: center ;" prop="collectionTime" :show-overflow-tooltip="true"/>
      <el-table-column label="回款金额" style="text-align: center ;" prop="collectionAmount" />
      <el-table-column label="单据编号" style="text-align: center ;" prop="documentNumber" :show-overflow-tooltip="true"/>
        ...
        <el-row :span="15">
          <el-col :span="8">
            <el-form-item label="收款类型" prop="payType">
              <el-select  v-model="form.payType" :disabled="true" placeholder="请选择收款类型" style="width: 100%;">
                <el-option
          ...
          <el-col :span="8" >
            <el-form-item label="发票信息" prop="invoiceId" v-if="fpvis">
              <!--start-->

              <el-input v-model="form.invoiceId" :readonly="true"  placeholder="请输入发票信息" >

                <el-button
                  slot="append"
                  icon="el-icon-search"
                  @click="getFpContractList"
                ></el-button>
              </el-input>
              <!--弹框-->
              <el-dialog
                ...
                <el-table  ref="fpTable"  v-loading="fploading" :data="fpcontractList" @selection-change="handleXmSelectionChange">
                  ...
                  <el-table-column label="项目编号" style="text-align: center ;" prop="projectNo" v-if="show"/>

                  <el-table-column label="合同编号" style="text-align: center ;" prop="contractNo" v-if="show"/>
        ...
        <el-form-item label="备注" prop="remark"  v-if="show">
         ...
         <el-col :span="8">
          <el-form-item label="银行单据" prop="voucher">
            ...
            >
              <el-button slot="trigger" size="small"  :disabled="authenStatus" type="primary">选取文件</el-button>
              <div slot="tip" class="el-upload__tip">
                只能上传jpg/png/pdf文件，且不超过20M
              </div>
            </el-upload>
          </el-form-item>
</el-col>
  ...
  methods: {
    ...
    exitFlow(row) {
      ...
      this.$confirm('是否确认终止本条数据?', "警告", {
        ...
        //exitFlow(instanceId);
        exitFlow(instanceId,'fpsh').then(response => {
          this.getReLists();
...
//获取项目信息
    ...
    getFpContractList() {
      ...
      this.queryParams.payType = 1;
      listReceiptInfo(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
        this.fpcontractList = response.rows;
        this.total = response.total;
    ...
    getReLists() {
      ...
      listReceiptInfo(this.queryParams).then(response => {
        ...
        this.financialLists = response.rows;
