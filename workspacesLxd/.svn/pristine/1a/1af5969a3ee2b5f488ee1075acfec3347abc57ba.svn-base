  ...
  <div class="app-container">
   ...
   <el-row :gutter="15">
    ...
    <el-table v-loading="loading" ref="itsmDataTable" border :data="collectionList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="合同金额(元)" align="center" prop="totalMoney" :show-overflow-tooltip="true"/>
      <el-table-column label="是否虚拟合同" align="center" prop="isVirtual" v-if="show" :show-overflow-tooltip="true" />
      <el-table-column label="是否老合同" align="center" prop="isOldCon"  v-if="show" :show-overflow-tooltip="true" />

      <el-table-column label="是否虚拟合同" align="center" prop="isVirtuals"  :show-overflow-tooltip="true" >
        <template slot-scope="s">
          <p v-if="s.row.isVirtual == 1">是</p >
          <p v-if="s.row.isVirtual == 2">否</p >
        </template>
      </el-table-column>
...
<!--               </div>-->
    ...
    <el-dialog title="发票申请" :visible.sync="open" width="80%" :close-on-click-modal="false">
...
<!--              ></el-button>-->
            ...
            <!--弹框-->
                  <el-dialog
                        ...
                        <el-table  ref="htTable" border v-loading="htloading" :data="contractList" @selection-change="handleXmSelectionChange">
        ...
        <el-col :span="8" v-if="yfkshow">
          <el-form-item label="" prop="yskamount" label-width="128px">
            ...
            <el-dialog
              ...
              <el-table  ref="htTable" border v-loading="htloading" :data="yskList" @selection-change="handleXmSelectionChange">
    ...
    <el-form ref="elForm" size="medium" label-width="100px">
      ...
      <el-table border v-loading="loading" border ref="itsmDataTables"  :data="ticketRecordList" @selection-change="handleXmSelectionChangeTick">
 ...
 /** 查看流程图 */
  ...
  handleAdd() {
    ...
    getUserInfoCon(_selectData[0].contractId).then(response => {
      ...
      this.title = "添加发票申请";
      this.dataList=[];
      this.isOldCon = _selectData[0].isOldCon;
  ...
  submithtForm(row) {
   ...
   // this.form.approvalType = row.projectCategory;
    ...
    if(row.isOldCon!=null && row.isOldCon!=0){
      this.isOldCon = row.isOldCon;
    }
  ...
  submitForm(val) {
    ...
    if(val==1){
      if(this.ticketRecordList==null && (this.form.isVirtual!=1 && this.form.isVirtual!='1')){
    ...
    this.$refs["form"].validate(valid => {
      ...
      if((_selectData==null || _selectData.length<=0)  && (this.form.isVirtual!=1 && this.form.isVirtual!='1')){
        this.msgError("请选择合同收款信息");
        this.saveBtn = false;
           return false;
      }
      ...
      let totalMoney = parseFloat(_selectData[0].totalMoney==null?0:_selectData[0].totalMoney);
      if(val==1){
        if((this.form.isVirtual==1 || this.form.isVirtual=='1')){
      ...
      if (valid) {
        ...
        this.form.isOldCon = this.isOldCon;

        if(this.isOldCon==1 && val==1){
          this.form.status = 2;
