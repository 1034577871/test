<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" ref="itsmDataTable" :data="contractList" @selection-change="handleSelectionChange" border >
      ...
      <el-table-column label="项目编号" align="center" prop="cbcProjectNo" width="130"/>
      <el-table-column label="项目名称" align="center" prop="cbcProjectName" />
      <el-table-column label="合同编号" align="center" prop="cbcContractNo" width="130"/>
      <el-table-column label="合同名称" align="center" prop="cbcContractName" />
      <el-table-column label="预计付款时间" align="center" prop="estimateTime" width="110"/>
      <el-table-column label="应付款金额" align="center" prop="estimateMoney" width="110">
        <template slot-scope="scope">
          {{scope.row.estimateMoney.toFixed(2) }}
        </template>
      </el-table-column>
      <el-table-column label="已收款金额" align="center" prop="actualMoney" width="110">
        <template slot-scope="scope">
          {{scope.row.actualMoney.toFixed(2) }}
        </template>
      </el-table-column>
    </el-table>
...
<script>
import { listContract,notReceivedWarning, getContract, delContract, addContract, updateContract, exportContract, collectionAndPaymentStatistics,contractWarningList } from '@/api/contract/contractInfo';
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      notReceivedWarning(this.queryParams).then(response => {
