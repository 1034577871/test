    ...
    <el-form ref="form" :model="form" label-width="100px">
      <el-dialog title="收票记录审核" :visible.sync="modalShow" width="80%" :before-close="modalClose">
        ...
        <el-row :gutter="15">
          <el-col :span="24" style="margin-top: 0;">
            <el-divider content-position="center">
              <el-link type="primary"><b>发票信息</b></el-link>
            </el-divider>
          </el-col>
        </el-row>
        <el-row :gutter="15">
  ...
  export default {
    ...
    methods: {
      init(row) {
        console.log(row)
        this.tableData = [];
        getTicketRecord(row.id).then(response => {
          this.modalShow = true;
          ...
          findFileListByPidPtype(row.id, 2).then(response => {
      ...
      //打印
      print_sp() {
        this.$print(this.$refs.ticketRecordInfo)
      },
    },
  }
</script>
