...
public class FormHistoryDataServiceImpl implements IFormHistoryDataService {
	...
	public List<HistoryDataDTO> historyDataShow(String businessKey) {
		...
		List<ActWorkflowFormData> actWorkflowFormData = actWorkflowFormDataService
				.selectActWorkflowFormDataByBusinessKey(businessKey);
//		Map<String, List<ActWorkflowFormData>> collect = actWorkflowFormData.stream()
//				.collect(Collectors.groupingBy(ActWorkflowFormData::getTaskNodeName));
		Map<String, List<ActWorkflowFormData>> collect = actWorkflowFormData.stream()
				.collect(Collectors.groupingBy(item -> item.getTaskNodeName() + "_" + item.getCreateName() + "_" + item.getCreateTime()));
		collect.entrySet().forEach(entry -> {
			...
			returnHistoryFromDataDTO.setFormHistoryDataDTO(entry.getValue().stream()
					.map(awfd -> new HistoryFormDataDTO(awfd.getControlName(), awfd.getControlValue()))
					.collect(Collectors.toList()));
			returnHistoryFromDataDTOS.add(returnHistoryFromDataDTO);
		});
		List<HistoryDataDTO> collect1 = returnHistoryFromDataDTOS.stream()
				.sorted((x, y) -> x.getCreatedDate().compareTo(y.getCreatedDate())).collect(Collectors.toList());
