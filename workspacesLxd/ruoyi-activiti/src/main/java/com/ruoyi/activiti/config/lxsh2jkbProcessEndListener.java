 ...
 */
@Component
public class lxsh2jkbProcessEndListener implements TaskListener, ExecutionListener {
    ...
    public void notify(DelegateTask delegateTask) {
        ...
        if(delegateTask.getVariable("pass")!=null){
            boolean pass=(boolean) delegateTask.getVariable("pass");
            //处理并行网关的多实例
            if(pass == false){
                //会签结束，设置参数result为N，下个任务为申请
                runtimeService.setVariable(exId, "jkbshstate", 1);
                //下个任务
                ...
                //说明都完成了并且没有人拒绝
                if((complete + 1) / all == 1){
                    runtimeService.setVariable(exId, "jkbshstate", 0);
                    //下个任务
                    String processInstanceId = delegateTask.getProcessInstanceId();
