package com.ruoyi.paymentManagement.payinfoDt.mapper;

import java.util.List;
import com.ruoyi.paymentManagement.payinfoDt.domain.CtBuPayinfoDt;

/**
 * 付款申请单明细Mapper接口
 * 
 * @author ruoyi
 * @date 2021-01-12
 */
public interface CtBuPayinfoDtMapper 
{
    /**
     * 查询付款申请单明细
     * 
     * @param id 付款申请单明细ID
     * @return 付款申请单明细
     */
    public CtBuPayinfoDt selectCtBuPayinfoDtById(String id);

    /**
     * 根据付款申请单ID查询
     *
     * @param bid 付款申请单ID
     * @return 付款申请单明细
     */
    public List<CtBuPayinfoDt> selectCtBuPayinfoDtByBid(String bid);

    /**
     * 查询付款申请单明细列表
     * 
     * @param ctBuPayinfoDt 付款申请单明细
     * @return 付款申请单明细集合
     */
    public List<CtBuPayinfoDt> selectCtBuPayinfoDtList(CtBuPayinfoDt ctBuPayinfoDt);

    /**
     * 新增付款申请单明细
     * 
     * @param ctBuPayinfoDt 付款申请单明细
     * @return 结果
     */
    public int insertCtBuPayinfoDt(CtBuPayinfoDt ctBuPayinfoDt);

    /**
     * 修改付款申请单明细
     * 
     * @param ctBuPayinfoDt 付款申请单明细
     * @return 结果
     */
    public int updateCtBuPayinfoDt(CtBuPayinfoDt ctBuPayinfoDt);

    /**
     * 删除付款申请单明细
     * 
     * @param id 付款申请单明细ID
     * @return 结果
     */
    public int deleteCtBuPayinfoDtById(String id);

    /**
     * 批量删除付款申请单明细
     * 
     * @param ids 需要删除的数据ID
     * @return 结果
     */
    public int deleteCtBuPayinfoDtByIds(String[] ids);


    /**
     * 根据bid逻辑删除付款申请单明细
     *
     * @param bid 付款申请单bid
     * @return 结果
     */
    public int updateCtBuPayinfoDtByBid(String bid);

    /**
     * 根据id逻辑删除付款申请单明细
     *
     * @param id 付款申请单id
     * @return 结果
     */
    public int updateCtBuPayinfoDtById(String id);
}
