<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="唯一ID" align="center" prop="isOldPro" v-if="show" />
      <el-table-column label="项目编号" align="center" prop="reqNo" width="150px" :show-overflow-tooltip="true"/>
      <el-table-column label="项目名称" align="center" prop="proName"  :show-overflow-tooltip="true"/>
      <el-table-column label="预计投资金额" align="center" prop="expectAmount" width="110px">
      <template slot-scope="s">
        {{s.row.expectAmount}}
      </template>
      </el-table-column>
      <!--      <el-table-column label="项目类型" align="center" prop="proTypeName" width="100px"/>-->
      <el-table-column label="公司是否垫资" width="110px">
        <template slot-scope="s">
          <p v-if="s.row.isAdvanceNeed == 'Y'">是</p >
          <p v-if="s.row.isAdvanceNeed == 'N'">否</p >
        </template>
      </el-table-column>
      <el-table-column label="是否经规划"  width="110px">
        <template slot-scope="s">
      ...
      <el-table-column
        ...
        :show-overflow-tooltip="true"
      />
      <el-table-column label="状态说明" align="center" prop="statusPs" width="100px" :show-overflow-tooltip="true"/>
      <el-table-column
        label="操作"
        align="center"
      ...
      >
        <template slot-scope="scope">
          ...
          <el-button
            ...
            type="text"
            icon="el-icon-delete"
            @click="handleDelete(scope.row)" v-if="roleBtn && (scope.row.status==0 || (scope.row.status==5))"
            v-hasPermi="['projectApproval:collectProject:remove']"
          >删除</el-button
          >
          ...
          <el-button
            size="mini"
            type="text"
            v-if="roleBtn && (scope.row.status!=4 && scope.row.status>=1 && scope.row.status!=5)"
            icon="el-icon-view"
          ...
          <el-button
            size="mini"
            type="text"
            icon="el-icon-view" v-if="scope.row.isOldPro!=1 && roleBtn && (scope.row.status>0)  && scope.row.status!=4 && scope.row.status!=5"
    ...
    <!-- 添加或修改事前审批主(集客类)对话框 -->
    <div ref="allLog" class="recordImg">
...
<!--              <el-radio-group v-model="form.isOldPro" size="mini" @change="isPro">-->
...
<!--              </el-radio-group>-->
            ...
            <el-col :span="12">
              <el-form-item label="项目名称" prop="proName"  label-width="130px">
                ...
                <el-input v-model="form.proName" placeholder="请输入项目名称"  id="proName"   :disabled="disabled"/>
                <el-input v-model="form.proName1"  v-if="show" placeholder="请输入项目名称" />

              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="预计投资金额(元)" label-width="125px" prop="expectAmount" >
                <el-input
            ...
            <el-col :span="12">
              <el-form-item label="规划部是否确认"  label-width="125px"  prop="isPlanConfirm" >
                <el-select
          ...
          <el-row v-if="ghb">
            <el-col :span="12">
              <el-form-item label="规划部预计确定投资金额(元)" prop="actualAmount" label-width="130px">
                <el-input
                  v-model="form.actualAmount"
                  clearable
                  placeholder="请输入规划部预计确定投资金额"
                  oninput="value=value.replace(/[^0-9.]/g,'')"
                  :disabled="pdisabled"
                />
              </el-form-item>
            ...
            <div class="no-print">
              <el-col :span="12" v-if="showDy" id="plan">
                <el-form-item
                  ...
                  >
                    ...
                    <div slot="tip" class="el-upload__tip">
                      只能上传jpg/png/jpeg/gif/pdf文件，且不超过20M
                    </div>
                  </el-upload>
          ...
          <div >
            ...
            <el-row style="margin-top: 15px;" v-if="showTables">
              ...
              <el-col :span="12" v-if="showUploads" class="no-print">
                <el-form-item
          ...
          <el-dialog title="事前审批"
                     ...
                     @close=""
          >
            <approvalInfo ref="approvalInfo" id="info" :approvalInfoDate="form.appxNgReq" :appxNgName="form.proName" :proType="form.proType"></approvalInfo>
            <div slot="footer" class="dialog-footer no-print">
              <el-button type="primary" v-if="bc2" :disabled="spBtn" @click="submitSpForm(0)">保 存</el-button>
          ...
          <el-dialog title="编辑信息"
            ...
            <div slot="footer" class="dialog-footer no-print" >
              <el-button type="primary" v-if="bc1" @click="submitFgsForm(3)" :disabled="fgsBtn">保 存</el-button>
              <el-button @click="editInfoVisible = false">取 消</el-button>
            </div>
          </el-dialog>
        </el-form>
        <div slot="footer" class="dialog-footer no-print"  v-if="butVis">
          <el-button type="success" v-if="bc3" @click="submitForms(0)" :disabled="btnDis">保 存</el-button>
          <el-button type="primary" v-if="tj3" @click="submitForms(4)" :disabled="btnDis">提 交</el-button>
          <el-button type="primary"    @click="print" v-if="dyBtn">打 印</el-button>
          <el-button @click="cancel">取 消</el-button>

        </div>
        <div slot="footer" class="dialog-footer no-print"  v-if="butVisi" >
          <el-button type="primary" v-if="tj5" @click="submitForms(10)">保 存</el-button>
          <el-button type="primary" v-if="tj5" @click="submitForms(5)">提 交</el-button>
          <el-button @click="cancel" v-if="qx5">取 消</el-button>
        </div>
        <div v-if="audit"  >
          <el-row :gutter="15" class="no-print">
            <el-col :span="24" >
              <el-divider content-position="center" ><b>审核信息</b></el-divider>
            </el-col>
          </el-row>
          <el-row :gutter="15">
            <el-col :span="24">

              <table class="curr-tables " border="1" style="border-color: #ddd;">
                <tbody>
                <tr>
                  <th width="30%">任务节点</th>
                  <th width="15%">审批人</th>
                ...
                <tr v-for="(value, index) in tableData_" :key="value">
                  <td align="center">
                    {{ value.taskNodeName }}
                  </td>
                  <td align="center">
  ...
  export default {
    ...
    data() {
      return {
        ...
        // 表单校验
        rules: {
          isOldPro: [{ required: true, message: '是否老项目不能为空', trigger: 'blur' }],
    ...
    methods: {
      isPro(obj){
        if(obj==1){
          this.reset();
          this.tj3 = false;
          this.form.isOldPro=1;
        }else{
          this.reset();
          this.form.isOldPro=2;
        }
      },
      changeCom(obj){
        if(this.form.isOldPro==1){
          return false;
        }
        this.$forceUpdate();
        let len = (this.form.selectComp.length);
        this.selLength = len;
        ...
        let objs = [];
        for(let obj_ of obj){
          let value = obj_.indexOf("(")+1;
          let val = obj_.substring(value,obj_.length-1);
          objs.push(val);
        }

       // v-model="dict.checkedState+'_'+dict.deptId"
        let listClone = this.belongCompanyNameList;
        this.isFileList = [];
        objs.forEach((item) => {
          listClone.forEach((dict) => {
      ...
      handleClickss(tab, event) {
        if(tab.name == 'base'){
           ...
           let mlist = this.$refs.materialsInfo.dataList;
          let mea =0;
           if(mlist!=null && mlist.length>0){
             for(let mite of mlist){
               mea=mea+parseFloat(mite.materialAmount==null?0:mite.materialAmount);
             }
           }
          let clist = this.$refs.construcostInfo.dataList;
          let con =0;
          if(clist!=null && clist.length>0){
            for(let cite of clist){
              con=con+parseFloat(cite.daysAmount==null?0:cite.daysAmount);
            }
          }
          this.$set(this.$refs.basicInfo.formData,'budgetMaterial',mea);
          this.$set(this.$refs.basicInfo.formData,'budgetLabor',con);
          this.$set(this.$refs.basicInfo.formData,'budgetTotal',parseFloat(mea)+parseFloat(con));
        }
      },
      changeBase(){
        this.$refs.basicInfo.formData.budgetMaterial  = this.$refs.materialsInfo.mates ;//将组件A传来的helloMsg赋予this.msg
      },
      ...
      print(){
        // this.$print(this.$refs.allLog) // 使
        this.$print(this.$refs.allLog)
        // let input = document.getElementById("proName").value;
        // document.getElementsByName("proName").value = input;
        //
        // document.getElementById("plan").style.display="none";
        // var print= document.getElementById('allLog');
        // var newContent = print.innerHTML;
        // var oldContent = document.body.innerHTML;
        // document.body.innerHTML = newContent;
        //
        ...
        // //将原有页面还原到页面
        //  document.getElementById("plan").style.display="bolck";
      ...
      selectChangedNeed(val){
        ...
        if(val=='Y' && this.form.isPlanConfirm=='Y'){
          if(this.form.selectComp.length==1){
            this.showTables = true;
            this.ghb = true;
          }else{
            this.showTables = false;
            ...
            selectCtBuApprovalNgReqById(id).then(response => {
              ...
              if(response.data.appxNgReq==null || response.data.appxNgReq.id==null ){
                ...
                if(storage_!=null){
                  ...
                  this.$refs.approvalInfo.formData  = approvalInfos;
                }else{
              ...
              let companyList = response.data.companyList;
              let clist = [];
              if(companyList!=null && companyList.length>0){
                for(let com of companyList){
                  clist.push(com.branchInvolvedId);
                }
              }
              let sgtotal = 0
              let cltotal=0;
              ...
              if(sessionKeys!=null && sessionKeys.length>0){
                if(companyList!=null){
                  for(let com of companyList){
                    ...
                    if(flag==0){
                      for(let ba of base){
                        if(com.id==ba.cid){
                          cls= cls+parseFloat(ba.budgetMaterial);
                          sgs= sgs+parseFloat(ba.budgetLabor);
                        }
                      }
                    }
                  }
                ...
                for(let key of sessionKeys){
                  ...
                  if(item!=null){
                    let itemJson = JSON.parse(item);//this.form.appNgBase
                    let appbase = (itemJson.appNgBase);
                    if(appbase!=null && appbase.budgetMaterial!=null){
                      cltotal = cltotal+parseFloat(appbase.budgetMaterial);
                      sgtotal = sgtotal+parseFloat(appbase.budgetLabor);
                    }
                  }

                }
                // this.$set(this.$refs.approvalInfo.formData,'stuffcostmoney',isNaN(cltotal)?0:cltotal);
                // this.$set(this.$refs.approvalInfo.formData,'buildcostmoney',isNaN(sgtotal)?0:sgtotal)

                this.$refs.approvalInfo.formData.stuffcostmoney = cltotal+cls;
                this.$refs.approvalInfo.formData.buildcostmoney = sgtotal+sgs;

              }else{
                for(let item of base){
                  cltotal = cltotal+parseFloat(item.budgetMaterial==null?0:item.budgetMaterial);
                  sgtotal = sgtotal+parseFloat(item.budgetLabor==null?0:item.budgetLabor);
                }
              }
              // }
            });
      ...
      openInfoWin(val){
        ...
        if(this.form.status>=1){
          ...
          getFxCollectProject(id,this.deptId_).then(response => {
            ...
            /** 基本信息赋值  budgetMaterial**/
            this.$set(this.$refs.basicInfo.formData,'budgetMaterial',null);
            this.$set(this.$refs.basicInfo.formData,'budgetLabor',null);
            this.$set(this.$refs.basicInfo.formData,'budgetTotal',null);
            ...
            if(response.data.appNgBase==null || response.data.appNgBase.id==null){
              if(storage==null){
                ...
                // this.$refs.basicInfo.formData.budgetMaterial=null;
                // this.$refs.basicInfo.formData.budgetLabor=null;s
                // this.$refs.basicInfo.formData.budgetTotal = null;
            ...
            this.$refs.construcostInfo.deptId= this.deptId_;
          });
        }else{
          // this.$refs["form"].validate(valid => {
          //   if (valid) {
        ...
        getCollectProject(id).then(response => {
          ...
          if( this.form.isOldPro==1){
            ...
            if((this.form.isAdvanceNeed=='Y' && this.form.isPlanConfirm=='Y')){
              ...
              if(this.form.actualAmount!='' && this.form.actualAmount!=null){
                if(parseFloat(this.form.actualAmount)>=500000){
      ...
      getList() {
        this.loading = true;
        listCollectProject(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
      ...
      handleUpdate_(row) {
        ...
        if(_selectData==null || _selectData.length<=0){
          ...
          pro = _selectData[0].isOldPro;
          statu = _selectData[0].status;
        }
        if(pro==1){
          this.reset();
          this.tj3=false;
          ...
          getCollectProject(id).then(response => {
            ...
            if(brans!='' && brans.length>0){
             ...
             // this.form.selectComp = dpData;
              this.$set(this.form,'selectComp',dpData)
            }
            getUserInfo().then(response1 => {
              if(response1.data.user.userName=='admin'){
                this.choise = false;
      ...
      submitForms(number) {
        ...
        if(this.form.isOldPro==1){
          ...
          let coms = this.form.selectComp;
          for(let c of coms){
            let comanyform = {
              branchInvolvedId:null,
            };
            let value = c.indexOf("(")+1;
            let val = c.substring(value,c.length-1);
            comanyform.branchInvolvedId =val;
        ...
        this.form.companys = [];
        // if(this.form.constructionPlan==null){
        //   this.form.constructionPlan=[];
        // }
        ...
        if(number==5){
          ...
          if (!this.$refs.field111.uploadFiles.length) {
            this.msgError("请上传附件");
            return false;
          }
        ...
        if(number==1 || number==4){
          ...
          if(this.form.isAdvanceNeed=='N' ){
            ...
            if(this.form.isAdvanceNeed=='Y' && this.form.isPlanConfirm=='N' ){
              ...
              if(this.form.isAdvanceNeed=='Y' && this.form.isPlanConfirm=='Y' ){
                if(this.form.actualAmount>=500000){
              ...
              if (this.form.id != null) {
                selectCtBuApprovalNgReqById(this.form.id).then(response => {
                  ...
                  if(oldBase==null || oldBase.length<=0){
                    ...
                    this.form.appSqsp = response.data.appxNgReq;
                    // comp.forEach((item, index) => {
                    //   let nextId = "";
      ...
      submitFgsForm(number) {
        ...
        //   if (valid) {
            if (this.$refs['basicInfo'].validateForm()) {
              //材料
              let budgetMaterial = this.form.appNgBase.budgetMaterial;
              let budgetMaterial_ = 0;
              if(this.form.appNgMaterialList !=null && this.form.appNgMaterialList.length>0){
                for(let item of this.form.appNgMaterialList){
                  budgetMaterial_ = budgetMaterial_+parseFloat(item.materialAmount);
                }
                this.$refs.basicInfo.formData.budgetMaterial=budgetMaterial_;
                this.form.appNgBase.budgetMaterial=budgetMaterial_;
                //this.form.appNgBase = this.$refs.basicInfo.formData;
              }else{
                this.msgError("请填写材料信息，基本信息中的材料预算将自动写入");
                this.fgsBtn = false;
                return false;
              }
              // if(budgetMaterial!=budgetMaterial_){
              //   this.msgError("材料预算不一致，请重新填写");
              //   this.fgsBtn = false;
              //   return false;
              // }
              //施工
              let budgetLabor = this.form.appNgBase.budgetLabor;
              let budgetLabor_ = 0;
              if(this.form.appNgConstList !=null && this.form.appNgConstList.length>0){
                for(let item of this.form.appNgConstList){
                  budgetLabor_ = budgetLabor_+parseFloat(item.daysAmount);
                }

                this.$refs.basicInfo.formData.budgetLabor=budgetLabor_;
                this.form.appNgBase.budgetLabor=budgetLabor_;
              }
              // if(budgetLabor!=budgetLabor_){
              //   this.msgError("施工预算不一致，请重新填写");
              //   this.fgsBtn = false;
              //   return false;
              // }
              // this.form.status = number;
              ...
              //总额
              this.$refs.basicInfo.formData.budgetTotal=budgetLabor_+budgetMaterial_;
              this.form.appNgBase.budgetTotal=budgetLabor_+budgetMaterial_;
              if((this.form.isAdvanceNeed=='Y' && this.form.isPlanConfirm=='Y')){
                if(this.form.selectComp.length==1){
                  this.form.actualAmount  =budgetLabor_+budgetMaterial_;
                }
              }
      ...
      handleDelete(row) {
        ...
        if(_selectData!=''){
          const flag = false;
          for(let item of _selectData) {
            if(item.status>0 && item.status!=5 && item.isOldPro!=1){
              deleSta = 1;
              break;
            }
          }
